...
export async function cloudWrite(familyId: string, data: FamilyData): Promise<void> {
  ...
  const content: Record<string, unknown> = {
    ...
    studyConfig: data.studyConfig ?? null,
    studyTemplates: data.studyTemplates || [],
    studyDayPlans: data.studyDayPlans || [],
    studyStreaks: data.studyStreaks || {},
    ...
    wordleDaily: data.wordleDaily ?? null,
    comicRecommendations: (data.comicRecommendations || []).slice(0, 100),
    mediaRecommendations: (data.mediaRecommendations || []).slice(0, 100),
    updatedAt: new Date().toISOString(),
  };
