...
import { ExternalLink, Film, Link2Off, RefreshCw } from 'lucide-react';
...
export function MediaPage() {
  ...
  const loadEmby = async () => {
    ...
    try {
      ...
      const [r, v] = await Promise.all([
        embyResume(embyUserId, 12),
        embyViews(embyUserId),
      ]);
      setResume(r);
      setViews(v);
  ...
  const openLibrary = (view: EmbyView) => {
    ...
    if (serverId) {
      window.open(
        embyWebLibraryLink(webUrl, serverId, view.Id),
        '_blank',
        'noopener,noreferrer',
      );
    } else {
